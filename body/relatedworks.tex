...
Topic drift detection is the core feature of MeetRX, enabling real-time alerts when a meeting conversation deviates from the agenda. The technical foundation for segmenting these shifts dates back to the \textbf{TextTiling Algorithm}\cite{hearst1997texttiling}, which utilizes a sliding window approach to compute lexical similarity between adjacent blocks of text to identify subtopic boundaries.

Modern implementations, such as the \textbf{TIAGE framework}\cite{tiage}, evolve this concept by using transformer-based embeddings and \textbf{cosine similarity} to detect changes in dialog. By comparing the vector representation of the live transcript window against the predefined agenda, systems can quantify `drift' more accurately than through keyword matching alone. Another study\cite{topic_shift} emphasizes that maintaining context is crucial in mixed-initiative conversations to prevent false positives in drift alerts.

Based on these findings, MeetRX implements a drift score computed via the embedding similarity between the current conversation window and the agenda. If the score falls below a specific similarity threshold, an alert is triggered.

\subsection{Agentic AI and Two-Tier System Design}
\label{subsection:agentic-ai-cascade}

Agentic AI allow systems to perform complex tasks automatically by integrating LLMs with toolchains\cite{code_analysis_agent}. However, the high computational cost and latency of large-scale models pose challenges for real-time systems.
